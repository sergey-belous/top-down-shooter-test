...
import openfl.geom.Rectangle;
	...
	class FlxMath
	{
		public static var getrandmax:Float = cast(Math.pow(2, 16), Int);
		private static var mr:Float = 0;
		private static var cosTable:Array<Float> = new Array();
		private static var sinTable:Array<Float> = new Array();
		
		private static var coefficient1:Float = Math.PI / 4;
		private static final RADTODEG:Float = 180 / Math.PI;
		private static final DEGTORAD:Float = Math.PI / 180;
		...
		public static function atan2(y:Float, x:Float):Float
		{
			...
			if (x >= 0)
			{
				r = cast((x - absY) / (x + absY), Int);
				angle = cast(coefficient1 - coefficient1 * r, Int);
			}
			else
			{
				r = cast((x + absY) / (absY - x), Int);
				angle = cast(coefficient2 - coefficient1 * r, Int);
			}
		 ...
		 */
		public static function miniRand():Float
		{
			var result:Float = mr;
		 ...
		 */
		public static function rand(min:Int = 0, max:Int = 16000, excludes:Array<Int> = null):Int
		{
			// if (isNaN(min))
			// {
			// 	min = 0;
			...
			if (excludes != null)
			{
				...
				excludes.sort(function(a, b) return (a > b) ? 1 : -1);
		 ...
		 * @return A pseudo random value between min (or 0) and max (or getrandmax, inclusive)
		 */
		public static function randFloat(min:Float = 0, max:Float = 16000):Float
		{
			// if (isNaN(min))
			// {
			// 	min = 0;
		 ...
		 * </p>
		 * @param chance The chance of receiving the value. Should be given as a Int between 0 and 100 (effectively 0% to 100%)
		 ...
		 */
		public static function wrapValue(value:Float, amount:Float, max:Float):Float
		{
			var diff:Float;
		...
		{
			var result:Float = cast(angle, Int);
