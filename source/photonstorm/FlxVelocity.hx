...
package photonstorm;
	...
	class FlxVelocity 
	{
		...
		public static function accelerateTowardsObject(source:FlxSprite, dest:FlxSprite, speed:Int, xSpeedMax:Int, ySpeedMax:Int):Void
		{
			...
			source.acceleration.x = cast(Math.cos(a) * speed, Int);
			source.acceleration.y = cast(Math.sin(a) * speed, Int);
		...
		public static function distanceBetween(a:FlxSprite, b:FlxSprite):Int
		{
			...
			return cast(FlxMath.vectorLength(dx, dy), Int);
		}
		...
		public static function velocityFromFacing(parent:FlxSprite, speed:Int):FlxPoint
		{
			...
			else if (parent.facing == RIGHT)
			{
				a = FlxMath.asRadians(0);
			}
			else if (parent.facing == UP)
			{
				a = FlxMath.asRadians( -90);
			}
			else if (parent.facing == DOWN)
			{
				a = FlxMath.asRadians(90);
			}
		...
		public static function angleBetweenMouse(a:FlxSprite, asDegrees:Bool = false):Float
		{
			...
			var p:FlxPoint = a.getScreenPosition();
